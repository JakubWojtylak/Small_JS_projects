...
function createDeck()
{
    ...
    let deck =[]; //pusta tablica
    for( let suitIdx = 0 ; suitIdx<suits.length; suitIdx++)
        for( let valueIdx = 0; valueIdx <values.length; valueIdx++)
        {
            let card= { //obiekt - karta
                suit : suits[suitIdx],
                value : values[valueIdx]
            };
            deck.push( card );// włóż karte do tablicy deck
...
document.getElementById("dealers").innerHTML= <img src="cards/card" +'('+dealerCards[0]
    ...
    {
        dealerCardString += getCardStr(dealerCards[i])+'\n';
    ...
    {
        playerCardString += getCardStr(playerCards[i])+'\n';
    }
